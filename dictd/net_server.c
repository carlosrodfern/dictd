	...
	*sock_fds_len = 0;
	...
	if (address_family == AF_UNSPEC) {
		...
		for (rtmp = r; rtmp != NULL; rtmp = rtmp->ai_next) {
			...
			sock = setup_socket(rtmp, queueLength, true, 0);
			break;
		}
		...
		if (sock == ESOOPTIPV6) { //dual stack failed, then open two sockets
			...
			for (rtmp = r; rtmp != NULL; rtmp = rtmp->ai_next) {
				if (rtmp->ai_family == AF_INET && sock_ipv4 == -1) {
					sock_ipv4 = setup_socket(rtmp, queueLength, false, 0);
					if (sock_ipv4 < 0) {
						err = sock_ipv4;
						break;
					}
				} else if (rtmp->ai_family == AF_INET6 && sock_ipv6 == -1) {
					sock_ipv6 = setup_socket(rtmp, queueLength, true, 1);
					if (sock_ipv6 < 0) {
						err = sock_ipv6;
						break;
					}
		...
		sock = net_open_tcp(address, service, queueLength, address_family);
		sock_fds = malloc(1*sizeof(int));
		sock_fds[0] = sock;
		*sock_fds_len = 1;
	}
